...
{"summary":"Codex summary","findings":[{"id":"same","severity":"blocker","title":"Bug","body":"Codex detail","path":"a.ts","line":2}],"intent_note":null}
JSON
  cat > "$work/norm-cursor.json" <<'JSON'
{"summary":"Cursor summary","findings":[{"id":"same","severity":"risk","title":"Bug","body":"Cursor detail","path":"a.ts","line":2}],"intent_note":null}
JSON
  ...
  eq "2" "$(jq '.findings[0].reviewers | length' "$work/merged.json")" || return 1
  jq -e '.summary | contains("Codex summary") and contains("Cursor summary")' "$work/merged.json" >/dev/null || return 1
  jq -e '.findings[0].body | contains("Codex detail") and contains("Cursor detail")' "$work/merged.json" >/dev/null || return 1
...
{"contributor":{"detected":true,"provider":"claude","label":"Claude","matches":6,"signal":"Co-Authored-By: Claude Opus 5"},"reviewers":[{"provider":"codex","label":"OpenAI Codex","model":"gpt-test"},{"provider":"cursor","label":"Cursor","model":"cursor-test"}]}
JSON
  local body
  ...
  printf '%s' "$body" | grep -q 'Coding-agent contributor detected.*Claude' || return 1
  printf '%s' "$body" | grep -q 'Independent reviewers asked in parallel' || return 1
  printf '%s' "$body" | grep -q 'OpenAI Codex' || return 1
  printf '%s' "$body" | grep -q 'Cursor' || return 1
  teardown
}
